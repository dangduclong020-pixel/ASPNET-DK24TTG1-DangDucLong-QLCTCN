/* ChiTieu JavaScript - Interactive Components */

// Initialize ChiTieu module when DOM is ready
document.addEventListener('DOMContentLoaded', function() {
    initializeChiTieu();
});

function initializeChiTieu() {
    // Initialize all ChiTieu components
...
function initializeFormValidation() {
    const forms = document.querySelectorAll('.chitieu-form');

    forms.forEach(form => {
        const inputs = form.querySelectorAll('.chitieu-form-control');
...
function validateInput(input) {
    ...
    switch(type) {
        case 'number':
            ...
            break;
        case 'date':
            // Loại bỏ validation ngày không được lớn hơn ngày hiện tại để cho phép chi tiêu dự kiến
            break;
        default:
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                entry.target.classList.add('chitieu-animate-fade-in');
            }
        });
    }, observerOptions);

    // Observe all cards
    document.querySelectorAll('.chitieu-card').forEach(card => {
        observer.observe(card);
    });

    // Add loading animations
    const loadingElements = document.querySelectorAll('.chitieu-loading');
...
function initializeTooltips() {
    ...
    const actionButtons = document.querySelectorAll('.chitieu-action-links a');
...
function showCustomTooltip(element, text) {
    const existingTooltip = document.querySelector('.chitieu-custom-tooltip');
    if (existingTooltip) {
        existingTooltip.remove();
    }

    const tooltip = document.createElement('div');
    tooltip.className = 'chitieu-custom-tooltip';
...
function hideCustomTooltip() {
    const tooltip = document.querySelector('.chitieu-custom-tooltip');
    if (tooltip) {
...
function initializeFilters() {
    const filterForm = document.querySelector('.chitieu-filter-form');
    ...
    const clearBtn = document.querySelector('.chitieu-clear-filters');
    if (clearBtn) {
...
function applyFilters() {
    const formData = new FormData(document.querySelector('.chitieu-filter-form'));
    ...
    // Show loading state
    const tableBody = document.querySelector('.chitieu-table tbody');
    if (tableBody) {
        tableBody.innerHTML = '<tr><td colspan="100%" class="text-center"><div class="chitieu-spinner"></div> Đang tải...</td></tr>';
    }
...
function initializeTableInteractions() {
    const table = document.querySelector('.chitieu-table');
    if (!table) return;
    ...
    // Select all checkbox
    const selectAll = document.querySelector('.chitieu-select-all');
    if (selectAll) {
        selectAll.addEventListener('change', function() {
            const checkboxes = document.querySelectorAll('.chitieu-row-checkbox');
    ...
    const rowCheckboxes = document.querySelectorAll('.chitieu-row-checkbox');
...
function updateBulkActions() {
    const checkedBoxes = document.querySelectorAll('.chitieu-row-checkbox:checked');
    const bulkActions = document.querySelector('.chitieu-bulk-actions');
...
document.addEventListener('change', function(e) {
    if (e.target.classList.contains('chitieu-auto-submit')) {
...
function initializePrint() {
    const printButtons = document.querySelectorAll('.chitieu-print-btn');
